...
function(currentRecord,email, record, runtime, search) {
    ...
    function reduce(context) {
    	try{
    		...
    		log.debug('value Length',valueLength);


 			for(var i=0; i< valueLength;i++){
 				var invoiceResult = JSON.parse(value[i]);
				log.debug('invoiceResult', invoiceResult);
				...
				if(r_invDueAmount>0){
					...
					var emailSubject='Pay Due Amount';
					log.debug('emailSubject',emailSubject);	
				    var emailBody = 'Hi Customer,' + '\r\n'
					+ 'Your Remaning amount to pay '+r_invAmount+' please create a full payment of Your Invoice Payment'+'\r\n' + 'Thanks,Admin';
			    
		 		    email.send({
						author : currentUser,
						recipients : r_custEmail,   //Recipient Email ID
						subject : emailSubject,   //Email subject
						body : emailBody            // Email Body From Upper emailBody Var
						});
		 		     log.debug('Email has sent Successfully',emailBody);
					}
				 else if(r_invDueAmount==0){
					alreadyPaid='This Customer has already Paid Full Amount'
					log.debug('alreadyPaid',alreadyPaid);
				}
